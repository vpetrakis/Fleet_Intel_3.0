...
ICONS = {
    "VERIFIED":     _u('<svg viewBox="0 0 28 28"><circle cx="14" cy="14" r="12" fill="none" stroke="#00e0b0" stroke-width="1" opacity=".2"/><circle cx="14" cy="14" r="7.5" fill="#061a14" stroke="#00e0b0" stroke-width="1.5"/><polyline points="10,14.5 12.8,17 18,10.5" fill="none" stroke="#00e0b0" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>'),
    "GHOST BUNKER": _u('<svg viewBox="0 0 28 28"><circle cx="14" cy="14" r="12" fill="none" stroke="#ff2a55" stroke-width="1" stroke-dasharray="4 3"/><circle cx="14" cy="14" r="7.5" fill="#1a0508" stroke="#ff2a55" stroke-width="1.5"/><g stroke="#ff2a55" stroke-width="2.5" stroke-linecap="round"><line x1="11" y1="11" x2="17" y2="17"/><line x1="17" y1="11" x2="11" y2="17"/></g></svg>'),
    "STAT OUTLIER": _u('<svg viewBox="0 0 28 28"><rect x="4" y="4" width="20" height="20" rx="5" fill="none" stroke="#c9a84c" stroke-width="1.2"/><circle cx="14" cy="14" r="4.5" fill="#0e0a1e" stroke="#c9a84c" stroke-width="1.5"/><circle cx="14" cy="14" r="1.8" fill="#c9a84c"/></svg>')
}
STATUS_COLORS = {"VERIFIED": "#00e0b0", "GHOST BUNKER": "#ff2a55", "STAT OUTLIER": "#c9a84c"}
...
def semantic_parse(file_bytes, file_name):
    ...
    MATH_COLS = ['FO_A', 'FO_L', 'MGO_A', 'MGO_L', 'Bunk_FO', 'Bunk_MGO', 'Bunk_MELO', 'Bunk_HSCYLO', 'Bunk_LSCYLO', 'Bunk_GELO', 'Bunk_CYLO', 'MELO_R', 'HSCYLO_R', 'LSCYLO_R', 'GELO_R', 'CYLO_R', 'Speed', 'DistLeg', 'TotalDist', 'CargoQty']
    for col in MATH_COLS: df[col] = df[col].apply(_sn)

    STRING_COLS = ['Voy', 'Port', 'AD', 'Date', 'Time']
    for col in STRING_COLS: df[col] = df[col].fillna("").astype(str).str.strip()
...
def build_state_machine(df, min_speed, ghost_sea, ghost_port):
    ...
    for i in range(len(ad_events) - 1):
        ...
        trips.append({
            ...
            'Daily_Burn': round(daily_burn, 1) if status == 'VERIFIED' else np.nan, 'MELO_L': round(melo_c, 0),
            'HSCYLO_L': round(hscylo_c, 0), 'LSCYLO_L': round(lscylo_c, 0), 'CYLO_GEN_L': round(cylo_gen_c, 0),
            'GELO_L': round(gelo_c, 0), 'Total_CYLO': round(hscylo_c + lscylo_c + cylo_gen_c, 0),
...
def chart_lube(df):
    fig = go.Figure()
    if df.get('MELO_L', pd.Series([0])).sum() > 0: fig.add_trace(go.Bar(x=df['Timeline'], y=df['MELO_L'], name='MELO', marker_color='rgba(0,224,176,0.15)', marker_line_color='#00e0b0', marker_line_width=1.5))
    if df.get('Total_CYLO',pd.Series([0])).sum() > 0: fig.add_trace(go.Bar(x=df['Timeline'], y=df['Total_CYLO'], name='CYLO (All)', marker_color='rgba(255,42,85,0.15)', marker_line_color='#ff2a55', marker_line_width=1.5))
    if df.get('GELO_L', pd.Series([0])).sum() > 0: fig.add_trace(go.Bar(x=df['Timeline'], y=df['GELO_L'], name='GELO', marker_color='rgba(201,168,76,0.15)', marker_line_color='#c9a84c', marker_line_width=1.5))
    fig.update_layout(**_BL, margin=_M, title=dict(text='Lubricant Consumption (Liters)', font=dict(size=24, family='Bricolage Grotesque', color="#fff")), barmode='group', showlegend=True, height=500, yaxis=dict(title='L', **_AX), xaxis=dict(automargin=True, **_AX))
...
def render_hud(sum_data):
    ...
    <div class="hud-grid">
        ...
        <div class="hud-card"><div class="hud-header"><div class="hud-title">Avg Sea Burn</div><div class="hud-icon">{svg_speed}</div></div><div class="hud-val">{sum_data['avg_sea_burn']:.1f}</div><div class="hud-sub">MT / Day</div></div>
        <div class="hud-card"><div class="hud-header"><div class="hud-title">Total MELO</div><div class="hud-icon">{svg_lube}</div></div><div class="hud-val">{int(sum_data['total_melo']):,}</div><div class="hud-sub">Liters</div></div>
        <div class="hud-card"><div class="hud-header"><div class="hud-title">Total CYLO</div><div class="hud-icon">{svg_lube}</div></div><div class="hud-val">{int(sum_data['total_cylo']):,}</div><div class="hud-sub">Liters</div></div>
        <div class="hud-card{w_class}"><div class="hud-header"><div class="hud-title">Anomalies</div><div class="hud-icon">{svg_alert}</div></div><div class="hud-val" style="color:{'#ff2a55' if sum_data['anomalies'] > 0 else '#fff'}">{sum_data['anomalies']}</div><div class="hud-sub">Flagged Deviations</div></div>
        <div class="hud-card{q_class}"><div class="hud-header"><div class="hud-title">Quarantined</div><div class="hud-icon">{svg_lock}</div></div><div class="hud-val" style="color:{'#ff2a55' if sum_data['quarantined'] > 0 else '#fff'}">{sum_data['quarantined']}</div><div class="hud-sub">Missing Data Legs</div></div>
    </div>
    """
...
if not files: st.info("Drop vessel noon-report files to execute the Multi-Dimensional Forensic Audit."); st.stop()
...
for f in files:
    with st.spinner(f'Auditing {f.name}…'):
        ...
        try:
            ...
            v_props = fleet_db.loc[vname] if vname in fleet_db.index else {}
    ...
    if err: st.error(f"**Rejected {f.name}:** {err}"); continue
    if trip_df.empty: st.warning(f"No valid events extracted from {f.name}. Check template schema."); continue
    ...
    st.markdown(f"""
    <div class="vcard" style="background: var(--glass-bg); backdrop-filter: blur(40px); border: 1px solid var(--glass-border); border-radius: var(--r); padding: 24px 32px; margin-bottom: 24px; box-shadow: var(--glass-shadow);">
        <div style="display:flex;justify-content:space-between;align-items:center">
            <div>
                <div style="font-family:var(--fd);font-weight:800;font-size:1.8rem; color:#fff;letter-spacing:-0.03em;">{sum_data['vname']}</div>
                <div style="font-family:var(--fm);font-size:.63rem;color:var(--t2); margin-top:6px;letter-spacing:0.08em">{sum_data['cycles']} LEGS&ensp;·&ensp;{sum_data['total_days']:.0f} DAYS&ensp;·&ensp;{int(sum_data['total_nm']):,} NM</div>
                <div style="font-family:var(--fm);font-size:.58rem;color:var(--acc2); margin-top:5px;letter-spacing:0.04em">{sum_data['ai_msg']}</div>
            </div>
            <div style="text-align:right">
                <div style="font-family:var(--fd);font-weight:800;font-size:2.8rem; color:{ic};line-height:1; text-shadow:0 0 28px rgba({r},{g},{b},0.45);">{sum_data['integrity']:.0f}%</div>
                <div style="font-family:var(--fm);font-size:.58rem;color:var(--t2); text-transform:uppercase;letter-spacing:.15em;margin-top:6px">Audit Integrity</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with t1:
        st.markdown('<div style="margin-bottom:14px"><span style="font-family:var(--fm);font-size:.72rem;color:var(--acc);background:rgba(0,224,176,0.05);padding:6px 12px;border-radius:6px;border:1px solid rgba(0,224,176,0.14);">[START ROB] + [BUNKERS] − [END ROB] = [PHYSICAL BURN]</span></div>', unsafe_allow_html=True)
        dcfg = {
            'Indicator': st.column_config.ImageColumn(' '), 'Timeline': st.column_config.TextColumn('TIMELINE', width='medium'),
            'Phase': st.column_config.TextColumn('LEG'), 'Days': st.column_config.NumberColumn('DAYS', format='%.2f'),
            'Speed_kn': st.column_config.NumberColumn('SPD', format='%.1f'), 'FO_A_Start': st.column_config.NumberColumn('START ROB', format='%.1f'),
            'Bunk_FO': st.column_config.NumberColumn('+ BUNKERS', format='%.1f'), 'FO_A_End': st.column_config.NumberColumn('− END ROB', format='%.1f'),
            'Phys_Burn': st.column_config.NumberColumn('= BURN', format='%.1f'), 'Log_Burn': st.column_config.NumberColumn('LOG BURN', format='%.1f'),
            'DQI': st.column_config.ProgressColumn('DQI', format='%d', min_value=0, max_value=100),
            'Daily_Burn': st.column_config.NumberColumn('MT/DAY', format='%.1f'), 'Total_CYLO': st.column_config.NumberColumn('CYLO (ALL)',format='%d'),
            'Status': st.column_config.TextColumn('STATUS', width='medium'),
        }
        st.dataframe(trip_df[['Indicator','Timeline','Phase','Days','Speed_kn','FO_A_Start','Bunk_FO','FO_A_End','Phys_Burn','Log_Burn','Drift_MT','Daily_Burn','Total_CYLO','MELO_L','GELO_L','DQI','Status']], column_config=dcfg, hide_index=True, use_container_width=True, height=500)
        buf = io.BytesIO()
        trip_df.drop(columns=['Indicator','Date_Start_TS'], errors='ignore').to_excel(pd.ExcelWriter(buf, engine='openpyxl'), index=False, sheet_name='Audit')
        buf.seek(0)
    ...
    with t2:
        voy = trip_df[~trip_df['Status'].str.contains('QUARANTINE')].groupby('Voy', dropna=False).agg(
            Total_Fuel=('Phys_Burn', 'sum'), Sea_Days=('Days', lambda x: x[trip_df.loc[x.index,'Phase']=='SEA'].sum()),
            Port_Days=('Days', lambda x: x[trip_df.loc[x.index,'Phase']=='PORT'].sum()), Sea_Fuel=('Phys_Burn', lambda x: x[trip_df.loc[x.index,'Phase']=='SEA'].sum()),
            Bunkers=('Bunk_FO', 'sum'), Dist=('Dist_NM', 'sum'), HSCYLO=('HSCYLO_L', 'sum'), LSCYLO=('LSCYLO_L', 'sum')
        ).reset_index()
    ...
    with t4:
        ...
        if 'AI_Exp' in sea_df.columns and sea_df['AI_Exp'].abs().sum() > 0:
            fig_c = go.Figure()
            fig_c.add_trace(go.Scatter(x=sea_df['Timeline'].tolist() + sea_df['Timeline'].tolist()[::-1], y=sea_df['Exp_Upper'].tolist() + sea_df['Exp_Lower'].tolist()[::-1], fill='toself', fillcolor='rgba(123,104,238,0.14)', line=dict(color='rgba(255,255,255,0)'), hoverinfo='skip', name='90% Conformal Interval'))
            fig_c.add_trace(go.Scatter(x=sea_df['Timeline'], y=sea_df['AI_Exp'], name='Expected Mean', line=dict(color='#7b68ee', width=2, dash='dash')))
            fig_c.add_trace(go.Scatter(x=sea_df['Timeline'], y=sea_df['Daily_Burn'], name='Audited Burn', mode='lines+markers', line=dict(color='#00e0b0', width=3), marker=dict(size=8, color='#051014', line=dict(color='#00e0b0', width=2))))
            fig_c.update_layout(**_BL, margin=_M, title=dict(text='Conformal Propulsion Bounds (Verified Sea Legs)', font=dict(size=22, family='Bricolage Grotesque', color='#fff')), height=700, yaxis=dict(title='MT/day', **_AX), xaxis=dict(tickangle=-45, automargin=True, **_AX))
            st.plotly_chart(fig_c, use_container_width=True, config={'displayModeBar': False})

    with t5:
        sea = trip_df[(trip_df['Phase'] == 'SEA') & (trip_df['Status'] == 'VERIFIED')]
        if 'HM_Base' in sea.columns and sea['HM_Base'].abs().sum() > 0:
            sel = st.selectbox('Select Verified Sea Passage', sea['Timeline'].tolist(), key=f'shap_{sum_data["vname"]}')
            tr = sea[sea['Timeline'] == sel].iloc[0]
            eb = tr['AI_Exp']

            fig_w = go.Figure(go.Waterfall(
                name="SHAP", orientation="v", measure=["absolute","relative","relative","relative","relative","relative","relative","total"],
                x=["Robust Baseline","Fleet Bias","Res. Speed","Mass","Kinematics","Season Spline","Degradation","AI Expected"], textposition="outside",
                text=[f"{tr['HM_Base']:.1f}", f"{tr['SHAP_Base']:+.1f}", f"{tr['SHAP_Propulsion']:+.1f}", f"{tr['SHAP_Mass']:+.1f}", f"{tr['SHAP_Kinematics']:+.1f}", f"{tr['SHAP_Season']:+.1f}", f"{tr['SHAP_Degradation']:+.1f}", f"{eb:.1f}"],
                y=[tr['HM_Base'], tr['SHAP_Base'], tr['SHAP_Propulsion'], tr['SHAP_Mass'], tr['SHAP_Kinematics'], tr['SHAP_Season'], tr['SHAP_Degradation'], 0],
                connector={"line": {"color": "rgba(255,255,255,0.08)", "width": 2, "dash": "dot"}}, decreasing={"marker": {"color": "#00e0b0"}}, increasing={"marker": {"color": "#ff2a55"}}, totals={"marker": {"color": "#7b68ee"}}
            ))
            fig_w.update_layout(**_BL, height=500, title=dict(text=f"Mathematical Delta Breakdown: {tr['Route']} ({tr['Speed_kn']}kn)", font=dict(size=20, family='Bricolage Grotesque', color='#fff')), yaxis=dict(**_AX), margin=dict(t=80, b=30, l=10, r=10))
            ...
            fig_stoch = go.Figure()
            fig_stoch.add_trace(go.Scatter(x=np.concatenate([x_fill, x_fill[::-1]]), y=np.concatenate([y_fill, np.zeros_like(y_fill)]), fill='toself', fillcolor='rgba(123,104,238,0.18)', line=dict(color='rgba(255,255,255,0)'), hoverinfo='skip', showlegend=False))
            fig_stoch.add_trace(go.Scatter(x=x_vals, y=y_vals, mode='lines', line=dict(color='rgba(123,104,238,0.9)', width=3), showlegend=False))
            fig_stoch.add_trace(go.Scatter(x=[eb, eb], y=[0, max(y_vals)], mode='lines', line=dict(color='#7b68ee', width=2, dash='dash'), showlegend=False))
            fig_stoch.add_trace(go.Scatter(x=[eb], y=[max(y_vals) * 1.15], mode='text', text=['AI Mean'], textfont=dict(color='#7b68ee', family='Geist Mono', size=13), showlegend=False))
            actual_color = '#00e0b0' if (tr['Daily_Burn'] >= tr['Exp_Lower'] and tr['Daily_Burn'] <= tr['Exp_Upper']) else '#ff2a55'
            ...
            fig_stoch.add_trace(go.Scatter(x=[tr['Daily_Burn']], y=[y_actual + max(y_vals) * 0.15], mode='markers+text', marker=dict(color=actual_color, size=14, symbol='diamond'), text=['Actual'], textfont=dict(color=actual_color, family='Geist Mono', size=13), textposition='top center', showlegend=False))
            fig_stoch.update_layout(**_BL, title=dict(text='Empirical Probability Density (Cross-Conformal)', font=dict(size=18, family='Bricolage Grotesque', color='#fff')), height=400, yaxis=dict(showticklabels=False, showgrid=False, zeroline=False), xaxis=dict(title='MT/day', **_AX), margin=dict(t=70, b=40, l=20, r=20))
            st.plotly_chart(fig_stoch, use_container_width=True, config={'displayModeBar': False})

            if tr['P_Value'] < 5.0: st.error(f"**Forensic Proof:** The Audited Burn falls at the absolute tail of the Conformal distribution. Empirical probability of natural occurrence: **{tr['P_Value']:.2f}%**. High probability of physical anomaly or mass extraction.")
            else: st.success(f"**Forensic Proof:** The Audited Burn is statistically nominal. Empirical probability of natural occurrence: **{tr['P_Value']:.2f}%**.")

            md_val, md_thresh = tr['Mahalanobis'], tr['MD_Threshold']
            md_color = '#00e0b0' if md_val <= md_thresh else '#ff2a55'
            fig_md = go.Figure(go.Indicator(
                mode="number+gauge", value=md_val, number={'font': {'color': md_color, 'size': 45, 'family': 'Bricolage Grotesque'}}, domain={'x': [0, 1], 'y': [0, 1]},
                title={'text': 'Kinematic Plausibility Matrix', 'font': {'size': 18, 'color': '#f8fafc', 'family': 'Bricolage Grotesque'}},
                gauge={
                    'axis': {'range': [None, max(md_val, md_thresh) * 1.2], 'tickwidth': 2, 'tickcolor': 'rgba(255,255,255,0.35)'}, 'bar':  {'color': md_color, 'thickness': 0.3},
                    'bgcolor': 'rgba(255,255,255,0.04)', 'borderwidth': 0, 'steps': [{'range': [0, md_thresh], 'color': 'rgba(0,224,176,0.14)'}, {'range': [md_thresh, max(md_val,md_thresh)*1.2], 'color': 'rgba(255,42,85,0.14)'}],
                    'threshold': {'line': {'color': '#fff', 'width': 3}, 'thickness': 0.82, 'value': md_thresh}
                }
            ))
            ...
            if md_val <= md_thresh: st.success(f"**Kinematic Audit: PASS.** The engine confirms this report — Mahalanobis distance ({md_val:.1f}) is within the historical threshold ({md_thresh:.1f}).")
            else: st.error(f"⚠️ **Kinematic Audit: FAIL.** Mahalanobis distance ({md_val:.1f}) exceeds the statistical threshold ({md_thresh:.1f}). The reported kinematic inputs are inconsistent with historical baseline.")
        else:
    ...
    with t6:
        ...
        if quar.empty: st.success("Zero anomalies. All timelines and mass balances intact.")
            ...
            for _, r in quar.iterrows():
                ...
                st.markdown(f'<div class="q-card"><span style="color:{c};font-weight:800;font-size:.78rem;letter-spacing:.1em">{r["Status"]}</span><span style="color:var(--t2);margin-left:14px;font-size:.78rem;font-family:var(--fm)">{r["Timeline"]}</span><div style="color:var(--t1);font-size:.82rem;margin-top:8px;font-weight:500">Exception: {r["Flags"]}</div></div>', unsafe_allow_html=True)

    st.divider()

if len(fleet_results) > 1:
    st.markdown('<h2 style="color:#fff;font-family:var(--fd);margin-top:20px;font-size:2rem">Fleet Comparison Matrix</h2>', unsafe_allow_html=True)
    ...
    st.dataframe(pd.DataFrame(fleet_rows), hide_index=True, use_container_width=True, height=350)
